...
Java_com_schnettler_outline_SubstratumLauncher_getAPKSignatureProduction(JNIEnv *env) {
    return (*env)->NewStringUTF(env, "4HCxqmQusuzbTuviIAOxy2sLSBA=");
}
...
Java_substratum_theme_template_SubstratumLauncher_getSamsungSupport(JNIEnv *env) {
    return Java_com_schnettler_outline_SubstratumLauncher_allowThirdPartySubstratumBuilds(env);
}
